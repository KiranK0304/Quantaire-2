...
async def _focus_chart(page: Page) -> None:
    ...
    try:
        ...
        if await chart_area.count() > 0 and await chart_area.first.is_visible():
            await chart_area.first.click(position={"x": 400, "y": 300})
...
async def set_timeframe(page: Page, tf: str) -> bool:
...
async def set_date_range(page: Page, date_range: str) -> bool:
    """
    ...
    try:
        ...
        all_inputs = await page.locator('[class*="dialog"] input').all()
        print(f"[timeframe] Dialog inputs found: {len(all_inputs)}")

        # Filter to only enabled date inputs (not the disabled time pickers)
        date_inputs = []
        for inp in all_inputs:
            is_disabled = await inp.get_attribute("disabled") is not None
            qa_id = await inp.get_attribute("data-qa-id") or ""
        ...
        if len(date_inputs) >= 2:
            ...
            await page.wait_for_timeout(200)
            await date_inputs[0].type(start_str, delay=50)
            await page.wait_for_timeout(300)
            ...
            await page.keyboard.press("Backspace")
            await page.wait_for_timeout(200)
            await date_inputs[1].type(end_str, delay=50)
            await page.wait_for_timeout(300)
            ...
            await all_inputs[0].click(click_count=3)
            await page.wait_for_timeout(200)
            await page.keyboard.press("Control+A")
            await page.keyboard.press("Backspace")
            await page.wait_for_timeout(200)
            await all_inputs[0].type(start_str, delay=50)
            await page.wait_for_timeout(300)
            
            await all_inputs[2].click(click_count=3)
            await page.wait_for_timeout(200)
            await page.keyboard.press("Control+A")
            await page.keyboard.press("Backspace")
            await page.wait_for_timeout(200)
            await all_inputs[2].type(end_str, delay=50)
            await page.wait_for_timeout(300)
...
async def zoom_out_chart(page: Page, steps: int = 10) -> None:
    ...
    try:
        ...
        if await chart_area.count() > 0 and await chart_area.first.is_visible():
            ...
            box = await chart_area.first.bounding_box()
